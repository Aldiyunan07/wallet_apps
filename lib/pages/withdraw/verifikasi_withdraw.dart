...
class VerifikasiWithdraw extends StatelessWidget {
  final int transactionId;

  const VerifikasiWithdraw({Key? key, required this.transactionId}) : super(key: key);
  ...
  void _confirmTransfer(String pin, BuildContext context) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        int transactionId = responseData['id'];
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => WithdrawSuccess(transactionId: transactionId),
          ),
        );
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            Text('Verifikasi Withdraw', style: TextStyle(color: Colors.white)),
